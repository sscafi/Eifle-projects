...
feature -- Type Queries
	...
	is_max_row(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value is within the maximum row range.
		require
			comment("etf_v: MAX_ROW = 3 .. 10")
		do
			Result := (3 <= etf_v) and then (etf_v <= 10)
		ensure
			Result = (3 <= etf_v) and then (etf_v <= 10)
		end

	is_max_column(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value is within the maximum column range.
		require
			comment("etf_v: MAX_COLUMN = 5 .. 30")
		do
			Result := (5 <= etf_v) and then (etf_v <= 30)
		ensure
			Result = (5 <= etf_v) and then (etf_v <= 30)
		end

	is_row(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value corresponds to a valid row identifier.
		require
			comment("etf_v: ROW = {A, D, C, J, F, E, B, G, I, H}")
		do
			Result := ((etf_v ~ A) or else (etf_v ~ D) or else (etf_v ~ C) or else (etf_v ~ J) or else (etf_v ~ F) or else
			           (etf_v ~ E) or else (etf_v ~ B) or else (etf_v ~ G) or else (etf_v ~ I) or else (etf_v ~ H))
		ensure
			Result = ((etf_v ~ A) or else (etf_v ~ D) or else (etf_v ~ C) or else (etf_v ~ J) or else (etf_v ~ F) or else
			          (etf_v ~ E) or else (etf_v ~ B) or else (etf_v ~ G) or else (etf_v ~ I) or else (etf_v ~ H))
		end
	...
	is_column(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value is within the column range.
		require
			comment("etf_v: COLUMN = 1 .. 30")
		do
			Result := (1 <= etf_v) and then (etf_v <= 30)
		ensure
			Result = (1 <= etf_v) and then (etf_v <= 30)
		end

	is_player_mov(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value is within the player movement range.
		require
			comment("etf_v: PLAYER_MOV = 1 .. 40")
		do
			Result := (1 <= etf_v) and then (etf_v <= 40)
		ensure
			Result = (1 <= etf_v) and then (etf_v <= 40)
		end

	is_project_mov(etf_v: INTEGER_32): BOOLEAN
		-- Check if the provided integer value is within the project movement range.
		require
			comment("etf_v: PROJECT_MOV = 1 .. 5")
		do
			Result := (1 <= etf_v) and then (etf_v <= 5)
		ensure
			Result = (1 <= etf_v) and then (etf_v <= 5)
		end
...
feature -- List of Enumerated Constants

	enum_items: HASH_TABLE[INTEGER, STRING]
		-- Hash table mapping integer constants to their corresponding string identifiers.
		do
	...
	enum_items_inverse: HASH_TABLE[STRING, INTEGER_32]
		-- Hash table mapping string identifiers to their corresponding integer constants.
		do
			create Result.make(10)
			Result.extend("A", 1)
			Result.extend("D", 2)
...
feature -- Query on Declarations of Event Parameters
	...
	evt_param_types_table: HASH_TABLE[HASH_TABLE[ETF_PARAM_TYPE, STRING], STRING]
		-- Hash table mapping event names to their respective parameter types.
		local
...
feature -- Query on Declarations of Event Parameters
	...
	evt_param_types_list: HASH_TABLE[LINKED_LIST[ETF_PARAM_TYPE], STRING]
		-- Hash table mapping event names to lists of their parameter types.
		local
...
feature -- Comments for Contracts
	...
	comment(etf_s: STRING): BOOLEAN
		-- Dummy method that always returns `TRUE` for comments in contracts.
		do
			Result := TRUE
		end
end
